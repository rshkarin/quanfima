...
def plot_orientation_map(
    ...
    quant_steps = 2056
    cmap_obj = cm.get_cmap(cmap_name, quant_steps)
    ...
    cb = mpl.colorbar.ColorbarBase(
        display_axes, cmap=cmap_obj, norm=norm, orientation="horizontal"
    )
    ...
    display_axes.text(
        ...
        transform=display_axes.transAxes,
    )
    ...
    if (output_dir is not None) and (name is not None):
        ...
        fig.savefig(
            os.path.join(output_dir, "{}_orientation_map.png".format(name)),
            transparent=False,
            bbox_inches="tight",
            pad_inches=0.1,
            dpi=dpi,
...
def plot_diameter_map(
    ...
    if (output_dir is not None) and (name is not None):
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        fig.savefig(
            os.path.join(output_dir, "{}_diameter_map.png".format(name)),
            transparent=False,
            bbox_inches="tight",
            pad_inches=0.1,
            dpi=dpi,
...
def gather_polar_errors(
    ...
    out = _angle_err2mean_abs_err(
        abs_err_azth, abs_err_lat, ref_azth, ref_lat, azth_rng=azth_rng, lat_rng=lat_rng
    )

    return out


def _angle_err2mean_abs_err(
    azth_err,
    lat_err,
    azth_ref,
    lat_ref,
    ...
    for i in xrange(len(azth_rng) - 1):
        for j in xrange(len(lat_rng) - 1):
            ...
            if len(idxs):
                mu_azth, mu_lat = np.rad2deg(np.mean(azth_err[idxs])), np.rad2deg(
                    np.mean(lat_err[idxs])
                )
...
def plot_polar_heatmap(
    ...
    f = interpolate.interp2d(
        rad, azm, data, kind="linear", bounds_error=True, fill_value=0
    )
    ...
    if log_scale:
        ...
        data_mesh = aux_ax.pcolormesh(
            new_th, new_r, new_data_angle_dist, cmap="jet", vmin=vmin, vmax=vmax
        )

    cbar = plt.colorbar(
        data_mesh, orientation="vertical", shrink=0.88, pad=0.1, aspect=15
    )
    ...
    if output_dir is not None:
        ...
        fig.savefig(
            os.path.join(output_dir, "{}_chart.png".format(name)),
            transparent=False,
            bbox_inches="tight",
            pad_inches=0.1,
            dpi=dpi,
    ...
    if color_limits:
        return 1.0, new_data_angle_dist.max()


def plot_histogram_fig(
    data,
    num_bins,
    xticks,
    color,
    ...
    n, bins, patches = ax.hist(
        data, num_bins, color=color, rwidth=bar_width, weights=weights
    )
    ...
    ax.ticklabel_format(
        axis="y", style="sci", scilimits=(-2, 2), fontsize=yticks_fontsize
    )
...
def create_pie_chart(
    data,
    rngs,
    colors=["#244268", "#426084", "#67809F", "#95A9C1", "#C6D2E0"],
    unit_scale=1.0,
    measure_quantity="m^3",
    figsize=(33, 15),
    legend_loc=(0.383, -0.25),
    zebra_color=(False, 3),
    legend_fontsize=50,
    chart_fontsize=60,
    dpi=72,
...
def _bbox_3D(img):
    ...
    print(np.where(r), np.where(c), np.where(z))
...
def plot_3d_orientation_map(
    ...
    azth, lat = azth_data[rmin:rmax, cmin:cmax, zmin:zmax], np.abs(
        lat_data[rmin:rmax, cmin:cmax, zmin:zmax]
    )
    ...
    #
    #     vv.screenshot(
    #         os.path.join(output_dir, "{}_3d_orientation.png".format(name)),
    #         sf=1,
    #         bg=background_color,
    #     )
    app.Run()
...
def plot_3d_diameter_map(
    ...
    if output_dir is not None:
        ...
        vv.screenshot(
            os.path.join(output_dir, "{}_3d_diameter.png".format(name)), sf=1, bg="w"
        )
    app.Run()
...
def plot_color_wheel(
    ...
    azth, lat = np.linspace(0.0, 1.0, num=180), np.linspace(0.0, 1.0, num=90)
    ...
    ax.tick_params(
        direction="out", length=2, width=0, labelsize=fontsize, pad=0, colors="w"
    )
    ...
    if output_dir is not None:
        ...
        fig.savefig(
            os.path.join(output_dir, "{}_color_bar.png".format(name)),
            transparent=True,
            bbox_inches="tight",
            pad_inches=0.1,
            dpi=dpi,
...
def plot_fourier_orientation(
    ...
    if (output_dir is not None) and (name is not None):
        ...
        plt.tight_layout()
        fig.savefig(
            os.path.join(output_dir, "{}_fourier_orientation_map.png".format(name)),
            bbox_inches="tight",
            transparent=True,
            pad_inches=0.05,
            dpi=dpi,
